...
describe('UpdateActionForm store redirect', () => {
  const { handleSuccess } = redirect;

  describe('handleSuccess()', () => {
    const getState = jest.fn();
    const action = { id: '00000000-0000-0000-0000-000000000000' };
    const response = { ok: true, json: { data: { action } } };
    ...
    it('should dispatch a push action', () => {
      const next = jest.fn();
      const dispatch = jest.fn();
      const url = `/mechanics/actions/${action.id}`;
