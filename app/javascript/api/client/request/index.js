...
const generateRequest = (options) => {
  ...
  const buildRequestWithMiddleware = applyMiddleware(
    buildRequest,
    selectMiddleware(actualMiddleware, 'buildRequest'),
  );

  const performRequest = (params = {}) => async (dispatch, getState) => {
    const {
      handleFailure,
      handlePending,
      handleSuccess,
