...
RSpec.describe Errors::FailedValidation do
  ...
  describe '#errors' do
    ...
    context 'when the record errors are empty' do
      let(:attributes) { FactoryBot.attributes_for(:spell) }
    ...
    context 'when the record has many errors' do
      ...
      let(:expected) do
        [
          ...
          ['level',       'must be less than or equal to 9'],
          ['name',        "can't be blank"]
        ]
      end
  ...
  describe '#message' do
    ...
    context 'when the record has many errors' do
      ...
      let(:expected) do
        "#{super()}: description can't be blank, level must be less than or" \
        " equal to 9, name can't be blank"
      end
